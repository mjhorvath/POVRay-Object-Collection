...
2.5.5
- Eliminated the divisions by zero errors that were being printed to the 
  Messages window.
- Fixed the bug where the crosswalk was being placed at the center of 
  the intersection when more than two roads were intersecting.

2.5.4
- Made it so that the crosswalk lies at the center of the intersection 
  when a flat intersection has only two roads that are parallel.

2.5.3
- Fixed the bug where the solid component parts of flat intersections 
  weren't being removed or hidden.
...
2.5.1
- Fixed several places in the demo scene file that hadn't been updated 
...
- The LRoad_WalkLength and LRoad_WalkWidth global variables have been 
  reversed.
...
- The placement of the crosswalks in intersections is now proper 
  with respect to the corresponding LDraw parts.
...
- Road coordinates of flat intersections are now specified using two-
  dimensional XZ coordinates instead of three-dimensional XYZ 
  coordinates.
